...
and pass `azure_client_id`/`azure_tenant_id`/`azure_subscription_id`). They
only make sense for a Terraform *root* config with a real backend — not a
reusable module repo like `vnet`/`aks`, which have no state of their own.
Nothing in this org uses them yet (there's no root-config repo, and no
Azure federated credential set up for CI); this is scaffolding for when a
demo/root repo exists:
